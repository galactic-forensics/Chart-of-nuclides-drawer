...
SIZE_FONT = 7
# Font size used for Z, A labels on axis
SIZE_FONT_AXL = 4
...
def load_xml_nuclear_table(
    datafile, n_range, z_range, n_limits=[None, None], z_limits=[None, None]
):
    ...
    for nuclide in dom.getElementsByTagName("nuclide"):
        try:
            ...
            if not (n_range[0] <= N <= n_range[1] and z_range[0] <= Z <= z_range[1]):
                continue
...
def _draw_triangle(layer, position, color, name, corner="rb"):
    ...
    if corner == "lt":
        ...
        x3 = x2
        y3 = y1
    triangle.setAttribute("points", "{},{} {},{} {},{}".format(x1, y1, x2, y2, x3, y3))
...
def _draw_text(layer, position, font_color, font_size, text):
    ...
    text_el.setAttribute(
        "style", "font-size:{}px; fill:{}".format(font_size, font_color)
    )
...
def draw_nuclide(nuclide, layers, position, args):
        ...
        if len(nuclide.decay_modes) > 1:
            ...
            for i in range(1, len(nuclide.decay_modes)):
                if nuclide.decay_modes[i]["mode"] in basic_decay_modes:
                    nuclide.decay_modes[0], nuclide.decay_modes[i] = (
                        nuclide.decay_modes[i],
                        nuclide.decay_modes[0],
                    )
                    primary_color = COLORS[nuclide.decay_modes[0]["mode"]]
                    break
            else:
    ...
    if len(nuclide.decay_modes) > 1:
        ...
        if len(nuclide.decay_modes) > 2 and (
            secondary_size == "large"
            or (secondary_size == "small" and primary_color == COLORS["is"])
        ):
            for i in range(2, len(nuclide.decay_modes)):
                try:
                    if nuclide.decay_modes[i]["mode"] in basic_decay_modes:
                        ...
                        re.search(cluster_re, nuclide.decay_modes[i]["mode"])
                        is not None
                    ):
                        tertiary_size = "small"
    ...
    _draw_rectangle(layers[0], position, primary_color, "{}0".format(nuclide))

    if secondary_size == "large":
        if secondary_color == COLORS["a"]:
            corner = "lt"
        elif (
            secondary_color == COLORS["b+"]
            and primary_color != COLORS["a"]
            and primary_color != COLORS["p"]
        ):
            corner = "lt"
        else:
            corner = "rb"
        _draw_triangle(
            layers[1], position, secondary_color, "{}1".format(nuclide), corner
        )
    ...
    if tertiary_size == "small":
        if tertiary_color == COLORS["a"]:
            ...
            tertiary_color == COLORS["b+"]
            and primary_color != COLORS["a"]
            and secondary_color != COLORS["a"]
        ):
            corner = "lt"
            ...
            corner = "rb"
        _draw_small_triangle(
            layers[1], position, tertiary_color, "{}2".format(nuclide), corner
        )
    ...
    if args.halflives and not (nuclide.half_life["extrapolated"] == "True"):
        ...
        if primary_color != COLORS["is"]:
            ...
            if nuclide.half_life["value"] != "?":
                ...
                if nuclide.half_life["relation"] != "=":
                    half_life_string = (
                        nuclide.half_life["relation"] + " " + half_life_string
                    )
        else:
        ...
        _draw_text(layers[3], [tx, ty], font_color, SIZE_FONT_HL, half_life_string)


def draw_magic_lines(layers, n_magic, z_magic, n_limits, z_limits, size):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "outfile", type=argparse.FileType("w"), help="Output data SVG file (required)"
    )
    parser.add_argument("--names", action="store_false", help="Disable names")
    parser.add_argument("--halflives", action="store_false", help="Disable half-lives")
    parser.add_argument("--magic", action="store_false", help="Disable magic numbers")
    parser.add_argument(
        "--numbers", action="store_false", help="Disable numbers along axis"
    )
    parser.add_argument(
        "--unknown",
        action="store_false",
    ...
    parser.add_argument(
        ...
        dest="Z",
        type=int,
        help="Atomic number Z range (%(type)s), default: %(default)s",
    )
    parser.add_argument(
        "--n",
        nargs=2,
        ...
        dest="N",
        type=int,
        help="Neutron number N range (%(type)s), default: %(default)s",
    )
    ...
    doctype_svg = dom.createDocumentType(
        "svg",
        "-//W3C//DTD SVG 1.1//EN",
    ...
    # left and bottom
    size = [
        (n_limits[1] - n_limits[0] + 2) * SIZE_FIELD + SIZE_GAP,
